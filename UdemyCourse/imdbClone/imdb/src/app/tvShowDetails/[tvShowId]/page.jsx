...
const MovieDetails = async ({params: {tvShowId}}) => {
    const {show, trailerKey} = await getMovieDetails(tvShowId);
    return (
        <div className='w-3/4 mx-auto p-2 border-x-2 border-black dark:border-x-2 dark:border-white'>
            ...
            <div className='flex'>
                ...
                <iframe className="w-full h-96" src={`https://www.youtube.com/embed/${trailerKey}?autoplay=1`} title={show.title || show.name} allowFullScreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>
            </div>
            <p>{show.overview}</p>
        </div>
)}
